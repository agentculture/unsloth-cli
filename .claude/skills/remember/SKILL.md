...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope unsloth-cli
...
scope:** the default personal scope is **private** (`--scope unsloth-cli
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `unsloth-cli`). Private records are served only to a recall in the same
