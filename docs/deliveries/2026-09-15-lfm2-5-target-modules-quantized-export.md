...
- PR #20 review (Qodo, 15 inline threads) after this summary was first written: 12 fixes landed — `--base` override now honoured via a staged adapter copy, `--output` may not overlap the adapter, non-directory `--output` rejected, ambiguous multi-GGUF eval dirs rejected (`--model <file.gguf>`), eval scores the continuation only (a pre-existing `run_eval` bug), awq/nvfp4 disk estimate covers the merged intermediate, malformed config fields tolerated, exports.json writes locked, corrupt export records render defensively, gguf export.json records the resolved default quant, eval `--model` needs the export home only for GGUF; three pushbacks (stdout chatter = risk r10, the finetune skill is first-party not vendored, Base vs Instruct is the requested #16 target, now annotated in the example). Deltas `b9`, `b10`.
...
- PR #21 review (Qodo): `e29`, `e30` and `e33` described more than their single cited test asserts. Narrowed replacements filed — `e35`, `e36`, `e37`, `e38`, `e39`, `e40` — one test per assertion (index locking, model-eval result fields and continuation scoring, and the manual docs/tested.md review as its own manual record). `e29`, `e30`, `e33` are superseded and await the owner's `devague evidence --reject`.
